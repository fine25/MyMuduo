    ...
    : looping_(false), quit_(false), callingPendingFunctors_(false), threadId_(CurrentThread::tid()), poller_(Poller::newDefaultPoller(this)), wakeupFd_(createEventfd()), wakeupChannel_(new Channel(this, wakeupFd_))
{
    ...
    // 每一个eventloop都会去监听wakeupchannel的EPOLLIN 读事件了·
...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
        ...
        {
            // Poller监听那些channel发生事件了，然后上报给EventLoop，通知cahnnel处理相应的事件
...
// 退出事件循环
// 情况一：loop在自己的线程中调用quit 二：在其他线程中（非loop ）调用quit
void EventLoop::quit()
{
    quit_ = true;
    if (!isInLoopThread())
...
void EventLoop::runInLoop(Functor cb)
{
    ...
    else // 在非当前loop线程中执行cb，就要唤醒loop所在的线程，执行cb
    {
        queueInLoop(cb);
    }
}
